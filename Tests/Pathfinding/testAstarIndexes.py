...
class testAStarIndexes(baseTest):
    ...
    def testDistanceGraphFindPathOverSubsector(self):
        sourcefile = self.unpack_filename('DeltaFiles/Zarushagar-Ibara.sec')
        ...
        approx = galaxy.trade.shortest_path_tree.reduced_distances()

        exp_route = [0, 8, 9, 15, 24, 36]
        act_route = galaxy.trade.shortest_path_tree._graph.find_shortest_path(source.index, target.index, approx=approx)
        self.assertEqual(exp_route, act_route)
