...
class TradeCalculation(RouteCalculation):
    ...
    def __init__(
        self, galaxy, min_btn=13, route_btn=8, route_reuse=10, debug_flag=False
    ):
        ...
        self.sector_passenger_balance = TradeBalance(
            stat_field="passengers", region=galaxy
        )
        # Track inter-sector trade imbalances
        self.sector_trade_balance = TradeBalance(
            stat_field="tradeExt", region=galaxy, target="trade"
        )
        # Track inter-allegiance passenger imbalances
        self.allegiance_passenger_balance = TradeBalance(
            stat_field="passengers",
            region=galaxy,
            field="alg",
        ...
        self.allegiance_trade_balance = TradeBalance(
            stat_field="trade",
            region=galaxy,
            field="alg",
            star_field="allegiance_base",
    ...
    def base_range_routes(self, star, neighbor):
        ...
        max_dist = self.btn_range[
            min(max(0, max(star.wtn, neighbor.wtn) - self.min_wtn), 5)
        ]
        ...
        if dist <= max_dist:
            ...
            if btn >= self.min_btn:
                ...
                self.galaxy.ranges.add_edge(
                    star, neighbor, distance=dist, btn=btn, passenger_btn=passBTN
                )
        return dist
    ...
    def generate_routes(self):
        ...
        for star in self.galaxy.stars:
            if len(self.galaxy.stars[star]) < 11:
                continue
            neighbor_routes = [
                (s, n, d) for (s, n, d) in self.galaxy.stars.edges([star], True)
            ]
        ...
        self.logger.info(
            "Final route count {}".format(self.galaxy.stars.number_of_edges())
        )
    ...
    def calculate_routes(self):
        ...
        btn_skipped = [
            (s, n)
            for (s, n) in self.galaxy.ranges.edges()
            if s.component != n.component
        ]
        self.logger.debug(
            f"Found {len(btn_skipped)} non-component routes, removing from ranges graph"
        )
        ...
        for star, neighbor, data in btn:
            if base_btn != data["btn"]:
                if counter > 0:
                    self.logger.info(
                        "processed {} routes at BTN {}".format(counter, base_btn)
                    )
                base_btn = data["btn"]
                counter = 0
            if total > 100 and processed % (total // 20) == 0:
                self.logger.info(
                    "processed {} routes, at {}%".format(
    ...
    def get_trade_between(self, star, target):
        ...
        if self.debug_flag:
            ...
            assert 1e-16 > delta * delta, (
                "Route weight between "
                + str(star)
                + " and "
                + str(target)
                + " should not be direction sensitive.  Forward weight "
                + str(fwd_weight)
                + ", rev weight "
                + str(rev_weight)
                + ", delta "
    ...
    def update_statistics(self, star, target, tradeCr, tradePass):
        if star.sector != target.sector:
            ...
            if star.subsector() == target.subsector():
                ...
                star.sector.subsectors[star.subsector()].stats.tradeExt += tradeCr // 2
                target.sector.subsectors[target.subsector()].stats.tradeExt += (
                    tradeCr // 2
                )
    ...
    def route_update_skip(self, route, tradeCr):
        ...
        for end in route[1:]:
            ...
            if self.galaxy.routes.has_edge(start, end) and self.galaxy.routes[start][
                end
            ].get("route", False):
                routeDist, routeWeight = self.route_update(
                    self.galaxy.routes[start][end]["route"], tradeCr
                )
                dist += routeDist
                weight += routeWeight
                usesJumpRoute = True
    ...
    def cross_check_totals(self):
        ...
        assert (
            grand_total_pax == total_sector_pax + self.sector_passenger_balance.sum
        ), "Sector total pax not balanced with galaxy pax"
        assert (
            grand_total_trade == total_sector_trade + self.sector_trade_balance.sum
        ), "Sector total trade not balanced with galaxy trade"
        assert (
            grand_total_pax
            == total_allegiance_pax + self.allegiance_passenger_balance.sum
        ), (
            "Allegiance total pax "
            + str(total_allegiance_pax + self.allegiance_passenger_balance.sum)
            + " not balanced with galaxy pax "
            + str(grand_total_pax)
        )
        assert (
            grand_total_trade
            == total_allegiance_trade + self.allegiance_trade_balance.sum
        ), "Allegiance total trade not balanced with galaxy trade"
